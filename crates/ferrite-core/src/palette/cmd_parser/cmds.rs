...
pub static COMMANDS: LazyLock<Vec<CommandTemplate>> = LazyLock::new(|| {
    let mut cmds = vec![
        ...
        CmdBuilder::new("relative-path", None, true).build(|_| Cmd::RelativePath),
        ...
        CmdBuilder::new("write-all", None, true).build(|_| Cmd::WriteAll),
        ...
        CmdBuilder::new("write", Some(("path", CmdTemplateArg::Path)), true).add_alias("w").build(|args| Cmd::Write {path: args[0].take().map(|arg| arg.unwrap_path()) }),
        CmdBuilder::new("write!", Some(("path", CmdTemplateArg::Path)), true).add_alias("w!").build(|args| Cmd::ForceWrite {path: args[0].take().map(|arg| arg.unwrap_path()) }),
        ...
        CmdBuilder::new("unicode", Some(("code-point", CmdTemplateArg::String)), false).build(|args| Cmd::UnicodeInsert { code_point: args[0].take().unwrap().unwrap_string() }),
