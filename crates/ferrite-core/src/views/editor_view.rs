...
use ferrite_geom::{point::Point, rect_ext::RectExt};
use ferrite_runtime::View;
...
impl View<Engine> for EditorView<'_> {
    fn layout(&self, state: &mut Engine) {
        // pass
    }

    fn handle_mouse(&self, state: &mut Engine) {
        // pass
    }
...
impl StatefulWidget for EditorView<'_> {
    ...
    fn render(
        ...
        buf.set_style(area, theme.background.into());
        ...
        let view = buffer.get_buffer_view(view_id);
        {
                        ...
                        match event {
                            HighlightEvent::Source { start, end } => {
                                if intersects(*start, *end, range.start, range.end) {
                                    ...
                                    {
                                        style = self.theme.get_syntax(name).into();
                                    }
            ...
            for (col, line) in dim_cells {
                ...
                buf.set_style(cell_area, theme.dim_text.into());
            }
                    ...
                    if (area.left().into()..area.right().into()).contains(&real_col) {
                        for y in area.top()..(area.bottom() - 1) {
                            ...
                            if cell.symbol().chars().all(|ch| ch.is_whitespace()) {
                                cell.set_symbol("│");
                                cell.set_style(theme.ruler.into());
                            }
                        }
                    }
                }
            ...
            for (col, line) in ruler_cells {
                let cell = buf.cell_mut((col, line)).unwrap();
                cell.set_char('│');
                cell.set_style(self.theme.ruler.into());
            }
            ...
            if self.config.highlight_cursor_line && draw_cursor_line && has_focus {
                ...
                if line_idx > start_line && line_idx < end_line {
                    ...
                    buf.set_style(cursor_line_area, theme.cursorline.into());
                }
            }
...

            {
                ...
                for (start, middle, end) in &*conflicts {
                    if intersects(*start, *end, start_line, end_line) {
                        ...
                        buf.set_style(first_area, theme.cursorline.into());
                        buf.set_style(second_area, theme.cursorline.into());
                    }
                }
            }
                        ...
                        .enumerate()
                    {
                        ...
                        if rect.area() > 0 {
                            ...
                            let style = if i == buffer.views[view_id].completer.index {
                                theme.completer_selected.into()
                            } else {
                                theme.completer.into()
                            };
